...
<%--                 <img style="width: 150px; height: auto;" class="h-10" src="<c:url value ="/resources/images/logo-removebg-preview.png"/>" alt="Logo"> --%>
    ...
    <div class="flex flex-wrap justify-end items-center w-full bg-gray-200">
		...
		<div class="table-wrapper">
			<a href = "<%=request.getContextPath()%>/showFormServiceInfo"><button style="text-align: left; margin: 4px; background-color: #FF99CC; color: white; padding: 3px 5px; border: 1px solid #CC99CC; border-radius: 5px;">Thêm dịch vụ</button></a>	
				...
				<tbody>
					<c:forEach items="${services}" var="item">
						<tr>
							...
							<td><c:out value="${item.name_service}" /> </td>
							<td>
							<a class="bx bxs-edit bx-xs"
								style="text-decoration: none; color: green"
								href="<%=request.getContextPath()%>/showFormServiceInfo?id_service=${item.id_service}">
							</a> &nbsp &nbsp <a class="bx bxs-trash bx-xs"
								style="text-decoration: none; color: red" href="#"
								onclick="confirmDelete(${item.id_service})"> </a></td>
						</tr>
					</c:forEach>
				<tbody>
			</table>
	...
	function confirmDelete(id_service) {
	  if (confirm('Bạn có chắc chắn muốn xóa dịch vụ này?')) {
	    ...
	    window.location.href = 'adminDeleteService?id_service=' + id_service;
	  } else {
	  }
	}
	</script>
